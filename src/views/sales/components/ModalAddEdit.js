...
const ModalAddEdit = ({ show = false, onHide = () => {} }) => {
  ...
  const categories = [
    { value: 'Trabajo', label: 'Trabajo', color: 'border-primary' },
    { value: 'Personal', label: 'Personal', color: 'border-tertiary' },
    { value: 'Otros', label: 'Otros', color: 'border-secondary' },
  ];
  ...
  return (
    <>
      <Modal className="modal-right fade" show={show} onHide={onHide}>
        <Modal.Header>
          <Modal.Title>{selectedEvent.id !== 0 ? 'Editar evento' : 'Agregar evento'}</Modal.Title>
        </Modal.Header>
          ...
          {selectedEvent.id !== 0 ? (
            <>
              ...
              <Button className="btn-icon btn-icon-end" onClick={saveItem}>
                <span>Guardar</span> <CsLineIcons icon="check" />
              </Button>
            </>
          ) : (
      ...
      <Modal className="fade modal-close-out" show={isShowDeleteConfirmModal}>
        ...
        <Modal.Body className="d-flex flex-column">
          <p>
            <span className="fw-bold">{selectedItem.title}</span> <span>será borrado. ¿Estás seguro?</span>
          </p>
        </Modal.Body>
        <Modal.Footer>
