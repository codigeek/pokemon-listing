...
const BrandsCategoryList = () => {

  const { currentBrand, currentBranch, currentUser, userType } = useSelector((state) => state.auth);

  const title = 'Categorías';
  const description = 'Gestionar categoría';
  const editURL = '/categoria-marca-detalle';
  const returnURL = '/catalogos/categorias_marca';
  const ApiURL = 'brand_category';
  ...
  useEffect(async() => {
    ...
    console.log("UsersTypes:",response);
...
export default BrandsCategoryList;
