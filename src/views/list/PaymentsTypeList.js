...
const PaymentsTypeList = () => {

  const { currentBrand, currentBranch, currentUser, userType } = useSelector((state) => state.auth);

  const title = 'Tipo de pagos';
  const description = 'Gestionar tipo de pago';
  const editURL = '/tipo-pago-detalle';
  const returnURL = '/administracion/tipos_pago';
